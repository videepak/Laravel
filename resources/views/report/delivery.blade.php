...
@section('content')
    <div class="right_col" role="main">
            <div class="row tile_count" style="text-align:center;margin-top: 7%;">
                <div class="col-md-offset-4 col-md-2 col-sm-4 col-xs-6 tile_stats_count">
                    <span class="count_top"><i class="fa fa-barcode"></i></i> Total Active Bin</span>
                    <div class="count total-bins"></div>
                    <span class="count_bottom"><i class="green"></i> ---------</span>
                </div>
                <div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
                    <span class="count_top"><i class="fa fa-barcode"></i></i> Total Serviced</span>
                    <div class="count total-pickup"></div>
                    <span class="count_bottom"><i class="green"></i> ---------</span>
                </div>
            </div>
                        ...
                        <table class="table table-striped jambo_table bulk_action" id="example">
                            <thead>
                                <tr class="headings">    
                                    ...
                                    <th class="column-title">Scan Date </th>
                                    <th class="column-title">
                                    <select class="form-control filter-search input-sm pull-right"
                                        style="width: 90%;" id="pickupType">
                                        <option value="">Activity</option>
                                        <option value='2'>Recycle/Waste Collection Done</option>
                                        <option value='3'>Violation Reported</option>
                                        <option value='5'>Rollback Activities</option>
                                        <option value='6'>Note Added</option>
                                        <option value='8'>Walk Through Done</option>
                                        <option value='11'>Route Check Point</option>
                                        <option value='12'>Redundant Route Service</option>
                                        </select>
                                    </th>
                                    <th class="column-title">Volume</th>
                                    <th class="column-title">
                                    <select class="form-control filter-search input-sm pull-right"
                                        style="width: 90%;" id="scanBy">
...
let pickup = $('#pickupType').val() != "" ? $('#pickupType').val()
...
let property = $('#pro-select2').val() != "" ? $('#pro-select2').val()
    ...
    $('#example').DataTable({
        ...
        language: {
            search: "",
            searchPlaceholder: "Search building or unit.",
...
if (new URLSearchParams(window.location.search).get('activity') != null) {
    let urlString = new URLSearchParams(window.location.search).get('activity');
    $("#pickupType").children('[value="' + urlString + '"]').prop("selected", true);
}
