...
class AutomatedServiceReportWeekly extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:AutomatedServiceReportWeekly';
    ...
    public function handle()
    {
        ...
        ->whereHas(
            ...
            function ($query) {
                $query->where('type', 9)
                    ->where('day_frequency', 2);
            }
        )
        ->get();

        if ($clock->isNotEmpty()) {
            automatedClockInOutReport($clock, 'Weekly');
        }
    #1352: Automated Clock in/out report: End
        ...
        if ($use->isNotEmpty()) {
            automatedUnitReport($use, 'Weekly');
        }
            ...
            if ($users->isNotEmpty()) {
                automatedServiceReport($users, 'Weekly');
            }
        ...
        if ($users->isNotEmpty()) {
            automatedViolationReport($users, 'Weekly');
        }
