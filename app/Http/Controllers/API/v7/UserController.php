...
class UserController extends Controller
{
    ...
    public function subProDetail(Request $request)
    {
        ...
        if ($notPickupList->count() > 0) {
            ...
            ->get()->count();
                ...
                //    dd($notPickupLists->service->pickup_type .'=='. '2' .'||'. $notPickupLists->service->pickup_type . '==' .'3');

                // if ($notPickupLists->get_employee_count == $notPickupLists->check_in_property_count) {
                ...
                if ($notPickupLists->type == 2 || $notPickupLists->type == 3) {
                    ...
                    $walk = \App\walkThroughRecord::select('id')
                     ...
                     ->get();
                    // dd($walk->count .'||'. $notPickupLists->get_unit_count .'=='. $proPickup);
                ...
                foreach ($buildings as $notBuilding) {
                    ...
                    $notBuilding['totalPickup'] = Activitylogs::where('type', 2)
    ...
    public function routeCheck(Request $request)
    {
        $validator = Validator::make(
            $request->all(),
            [
                'barcode_id' => 'required|exists:route_check_ins,barcode_id',
            ],
            [
        ...
        $property = \App\RouteCheckIn::select('property_id', 'building_id')
            ->where('barcode_id', $request->barcode_id)->first();
            ...
            $validator = Validator::make(
                ...
                [
                    ...
                    'barcode_id' => [
                        'required',
                        'string',
                        Rule::exists('route_check_ins', 'barcode_id'), //Task: #1069, Comment: #7
                    ],
                ],
                [
                    'barcode_id.exists' => 'Invalid Barcode',
    ...
    private function scanCheckpoint($request)
    {
        ...
        $validator = Validator::make(
            ...
            [
                'barcode_id' => [
                    'required', 'string',
                    Rule::exists('route_check_ins', 'barcode_id')
                        ->whereNull('deleted_at'),
                ],
                'lat' => 'required',
                'long' => 'required',
        ...
        $route = \App\RouteCheckIn::select('id', 'name', 'barcode_id', 'building_id', 'property_id', 'address1', 'address2')
        ...
        if (in_array($route->getProperty->id, $rerouteProperty) || !empty($request->reRoute)) {
            Activitylogs::create(
                [
                    'text' => 'Redundant Route Successful.',
                    'user_id' => $this->userId,
                    ...
                    'barcode_id' => $request->barcode_id,
                    'type' => 12,
                    'property_id' => $route->getProperty->id,
                    'building_id' => $route->getBuilding->id,
                    'latitude' => $request->lat,
                    'longitude' => $request->long,
                    'ip_address' => $request->ip(),
                ]
    ...
    public function createViolationV1(Request $request)
    {
        ...
        if (empty($request->type)) {
            ...
            $validator = Validator::make(
                ...
                [
                        ...
                        'type' => 'nullable|integer', //Task: #1069, Comment: #5
                        'barcode_id' => ['required', 'string',
                            Rule::exists('route_check_ins', 'barcode_id'), //Task: #1069, Comment: #5
                        ],
                ],
                [
                    'barcode_id.exists' => 'Invalid Barcode',
        ...
        if (!is_null($check)) {
            $data = [
                'responseCode' => '206', //Only change the"responseCode" 201 to 206., when "Violation has been created already" message occur in the "createViolationV1" APIs..
        ...
        if (empty($request->type) && !$this->isEmployeeValid($request->barcode_id)) {
            $data = [
                'responseCode' => 201,
                'responseMsg' => 'Invalid QR code.',
                'requestid' => $requestId,
        ...
        $property = $table::where('barcode_id', $request->barcode_id)->first();
        //Task: #1069, Comment: #5:End.

        $activityId = Activitylogs::create(
            [
                'text' => empty($request->type) ? 'Violation Reported' : 'Route Checkpoint Violation Reported',
                'user_id' => $this->userId,
                'updated_by' => $this->userId,
                'barcode_id' => $request->barcode_id,
                'type' => 3,
    ...
    {
        $route = \App\RouteCheckIn::select('barcode_id')
            ->where(
                [
    ...
    public function getEmployescheduleV3(Request $request)
    {
        ...
        $appPermission = \App\AppPermission::where('subscriber_id', $this->subscriberId)->where('user_id', $user->id)->first();
    ...
    /**
     * Get Employee Schedule V4.
     *
     * Version v7: Task: #1183 (without check-in).
     *
     * @response {
     * "message": "success",
     * "data": {
...
//        });
    ...
    private function getRemainingPickupPropertyV3($user_id, $date)
    {
        ...
        $notPickupList = \App\Property::select('name', 'address', 'city', 'state', 'zip', 'id', 'type', 'state')
            ->whereIn(
                'id',
        ...
        if ($notPickupList->isNotEmpty()) {
            foreach ($notPickupList as $key => $notPickupLists) {
                ...
                foreach ($buildings as $notBuilding) {
                    ...
                    $collection = Units::select('latitude', 'longitude', 'address1', 'address2', 'floor', 'building', 'unit_number', 'barcode_id', 'is_active', 'property_id', 'building_id', \DB::raw('"unit" as type'))
                        ->where('property_id', $notPickupLists->id)
                        ->when(
                    ...
                    $notBuilding['check_point'] = \App\RouteCheckIn::select('id', 'name as unit_number', 'address1', 'address2', 'barcode_id', 'building_id', 'property_id', 'is_required', \DB::raw('"routeCheckPoint" as type'))
                    ->where('property_id', $notPickupLists->id)
                    ->where('building_id', $notBuilding->building_id)
                    ->withCount(
                        [
