    ...
    */
    ...
    public function uploadViolationImage(Request $request)
    {
        ...
        foreach ($request->images as $file) {
            ...
            $existImage = \App\ViolationImages::
...
//                        })
                ...
                ->when($this->platform == 'ANDROID' || ($this->platform == 'IOS' && $this->appVersion >= 14), function ($query) use ($startDate, $user_id, $endDate) {
                    $query->whereHas('service', function ($query) use ($startDate, $endDate) {
                        $query->where('pickup_start', '<=', \Carbon\Carbon::now()->setTimezone($this->timezone))
                        ->where('pickup_finish', '>=', \Carbon\Carbon::now()->setTimezone($this->timezone));
                    })
        ...
        try {
            ...
            if ($mail->isNotEmpty() && $mail[0]->getEmployee->isNotEmpty()) {
                ...
                $data = [
                    ...
                    'checkInDate' => \Carbon\Carbon::now()->timezone($this->timezone)->format('m-d-Y'),
